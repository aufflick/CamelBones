...
can be used to provide baseURI when parsing strings or filehandles. This URI is
used when resolving relative URIs to external DTD subset, external entities, or
XInclude elements. This URI is returned by the C<<<<<< URI >>>>>> method of the C<<<<<< XML::LibXML::Document >>>>>> object. 
