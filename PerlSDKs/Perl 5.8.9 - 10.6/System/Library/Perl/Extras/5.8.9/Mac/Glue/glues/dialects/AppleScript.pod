...
Properties:
    ...
    id (ID  /shor): the Apple Desktop Bus device ID (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
...
Properties:
    ...
    conduit (pcon/econ): How the addressee is physically connected (read-only)
    properties (pALL/reco): property that allows getting and setting of multiple properties (read-only)
    protocol (pprt/epro): How to talk to this addressee (read-only)
...
Properties:
    ...
    appletalk_machine (patm/TEXT): the machine name part of the address (read-only)
    appletalk_type (patt/TEXT): the type part of the AppleTalk address (read-only)
    appletalk_zone (patz/TEXT): the zone part of the address (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
...
Properties:
    ...
    clipboard (pcli/****): the contents of the clipboard for this application (read-only)
    ...
    selection (sele/csel): the selection visible to the user.  Use the ‘select’ command to set a new selection; use ‘contents of selection’ to get or change information in the document. (read-only)
...
Properties:
    ...
    arc_angle (parc/fixd): the angle of the arc in degrees (read-only)
    bounds (pbnd/qdrt): the smallest rectangle that contains the entire arc (read-only)
    definition_rect (pdrt/qdrt): the rectangle that contains the circle or oval used to define the arc (read-only)
    fill_color (flcl/cRGB): the fill color (read-only)
    fill_pattern (flpt/cpix): the fill pattern (read-only)
    pen_color (ppcl/cRGB): the pen color (read-only)
    pen_pattern (pppa/cpix): the pen pattern (read-only)
    pen_width (ppwd/shor): the pen width (read-only)
    start_angle (pang/fixd): the angle that defines the start of the arc, in degrees (read-only)
    transfer_mode (pptm/tran): the transfer mode (read-only)
...
Properties:
    ...
    id (ID  /shor): the slot number (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
...
Properties:
    ...
    formula (pfor/ctxt): the formula of the cell (read-only)
    protection (ppro/prtn): Indicates whether value or formula in the cell can be changed (read-only)
...
Properties:
    ...
    name (pnam/itxt): the name of the column (read-only)
...
Properties:
    ...
    background_color (pbcl/cRGB): the color used to fill in unoccupied areas (read-only)
    background_pattern (pbpt/cpix): the pattern used to fill in unoccupied areas (read-only)
    color_table (cltb/clrt): the color table (read-only)
    default_font (ptxf/itxt): the name of the default font for text objects (read-only)
    default_location (pnel/QDpt): the default location of each new graphic object (read-only)
    default_size (ptps/fixd): the default size for text objects (read-only)
    name (pnam/itxt): the name (read-only)
    ordering (gobs/obj ): the ordered list of graphic objects in the drawing area (read-only)
    pixel_depth (pdpt/shor): the number of bits per pixel (read-only)
    rotation (prot/trot): the default rotation for objects in the drawing area (read-only)
    scale (pscl/fixd): the default scaling for objects in the drawing area (read-only)
    style (txst/tsty): the default text style for text objects (read-only)
    text_color (ptxc/cRGB): the default color for text objects (read-only)
    translation (ptrs/QDpt): the default repositioning for objects in the drawing area (read-only)
    update_on_change (pupd/bool): Redraw after each change? (read-only)
...
Properties:
    ...
    id (ID  /long): the Ethernet address (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
...
Properties:
    ...
    id (ID  /shor): the FireWire device ID (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
...
Properties:
    ...
    arrow_style (arro/arro): the arrow style (read-only)
    dash_style (pdst/tdas): the dash style (read-only)
    end_point (pend/QDpt): the ending point of the line (read-only)
    start_point (pstp/QDpt): the starting point of the line (read-only)
...
Properties:
    ...
    color (colr/cRGB): the color of the first character (read-only)
    font (font/ctxt): the name of the font of the first character (read-only)
    size (ptsz/fixd): the size in points of the first character (read-only)
    uniform_styles (ustl/tsty): the text styles that are uniform throughout the text (read-only)
...
Properties:
    ...
    dns_form (pdns/TEXT): the address in the form "apple.com" (read-only)
    id (ID  /TEXT): the address in the form "127.201.0.1" (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
    port (ppor/TEXT): the port number of the service or client being addressed (read-only)
...
Properties:
    ...
    id (ID  /long): the unique ID number of this object (read-only)
...
Properties:
    ...
    key (kMsg/cha ): the character for the key was pressed (ignoring modifiers) (read-only)
    key_kind (kknd/ekst): the kind of key that was pressed (read-only)
    modifiers (kMod/eMds): the modifier keys pressed in combination (read-only)
...
Properties:
    ...
    inheritance (c@#^/text): inherits some of its properties from this class (read-only)
    justification (pjst/just): the justification of the text (read-only)
...
Properties:
    ...
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
    network (pnet/shor): the LocalTalk network number (read-only)
    node (pnod/shor): the LocalTalk node number (read-only)
    socket (psoc/shor): the LocalTalk socket number (read-only)
...
Properties:
    ...
    color (colr/cRGB): the color (read-only)
...
Properties:
    ...
    point_list (ptlt/QDpt): the list of points that define the polygon (read-only)
...
Properties:
    ...
    corner_curve_height (pchd/shor): the height of the oval used to define the shape of the rounded corners (read-only)
    corner_curve_width (pcwd/shor): the width of the oval used to define the shape of the rounded corners (read-only)
...
Properties:
    ...
    id (ID  /shor): the SCSI ID (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
    lun (pslu/shor): the SCSI logical unit number (read-only)
    scsi_bus (pscb/shor): the SCSI bus number (read-only)
...
Properties:
    ...
    contents (pcnt/****): the information currently selected.  Use ‘contents of selection’ to get or change information in a document. (read-only)
...
Properties:
    ...
    size (ptsz/fixd): the size in points of the first character (read-only)
    style (txst/tsty): the text style of the first character of the first character (read-only)
    uniform_styles (ustl/tsty): the text styles that are uniform throughout the text (read-only)
...
Properties:
    ...
    id (ID  /shor): the Token Ring ID (read-only)
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
...
Properties:
    ...
    inheritance (c@#^/address_specification): inherits some of its properties from this class (read-only)
    name (pnam/TEXT): the USB device name (read-only)
...
Properties:
    ...
    bounds (pbnd/qdrt): the boundary rectangle for the window (read-only)
    closeable (hclb/bool): Does the window have a close box? (read-only)
    floating (isfl/bool): Does the window float? (read-only)
    index (pidx/long): the number of the window (read-only)
    ...
    visible (pvis/bool): Is the window visible? (read-only)
    zoomable (iszm/bool): Is the window zoomable? (read-only)
    zoomed (pzum/bool): Is the window zoomed? (read-only)
