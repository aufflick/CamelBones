...
class DgTestCase (unittest.TestCase):
    ...
    def compileTestCase(self):
        ...
        # commandline='cc -g -DMACOSX -Iinclude -o /tmp/test.bin %s %s 2>&1'%(self.filename, ' '.join(libffiobjects))
# BC      commandline='cc -g -DMACOSX -Iinclude -o /tmp/test.bin %s -lffi 2>&1'%(self.filename)
# BC adding "-arch ppc64" temporarily
        commandline='cc -g -DMACOSX -Iinclude -o /tmp/test.bin %s -lffi -arch x86_64 2>&1'%(self.filename)
