...
#ifdef __ppc__
                // On PPC, all structs should result in a call to CBPerlIMP_stret()
                NSLog(@"CBPerlIMP() called with struct return type: %s", returnType);
                returnValue.ulong = 0;
#endif

#ifdef __i386__
                // On i386, large structs should result in a call to CBPerlIMP_stret();
                NSGetSizeAndAlignment(returnType, &returnSize, &alignedSize);

                if (returnSize > 8) {
                    NSLog(@"CBPerlIMP() called with large struct return type: %s", returnType);
                    returnValue.ulong = 0;

                } else {

                    if (0 == strncmp(returnType, "{NSPoint", strlen("{NSPoint"))) {
                        NSPoint s = REAL_CBPointFromSV(POPs);
                        memcpy(returnValue.struct_buf, &s, returnSize);
                    } else if (0 == strncmp(returnType, "{NSRange", strlen("{NSRange"))) {
                        NSRange s = REAL_CBRangeFromSV(POPs);
                        memcpy(returnValue.struct_buf, &s, returnSize);
                    } else if (0 == strncmp(returnType, "{NSRect", strlen("{NSRect"))) {
                        NSRect s = REAL_CBRectFromSV(POPs);
                        memcpy(returnValue.struct_buf, &s, returnSize);
                    } else if (0 == strncmp(returnType, "{NSSize", strlen("{NSSize"))) {
                        NSSize s = REAL_CBSizeFromSV(POPs);
                        memcpy(returnValue.struct_buf, &s, returnSize);
                    } else {
...
void REAL_CBPerlIMP_stret(void* returnBuffer, id self, SEL _cmd, ...) {
    // Define a Perl context
    PERL_SET_CONTEXT(_CBPerlInterpreter);
    dTHX;
    ...
    if (success) {
        ...
        switch (*returnType) {
            case '{':
...
#ifdef __i386__
                // On i386, small structs should result in a call to CBPerlIMP();
                if (returnSize <= 8) {
                    NSLog(@"CBPerlIMP_stret() called with small struct return type: %s", returnType);
                    returnValue.ulong = 0;
                    break;
                }
#endif
                ...
                if (0 == strncmp(returnType, "{NSPoint", strlen("{NSPoint"))) {
                    NSPoint s = REAL_CBPointFromSV(POPs);
                    memcpy(returnBuffer, &s, returnSize);
                } else if (0 == strncmp(returnType, "{NSRange", strlen("{NSRange"))) {
                    NSRange s = REAL_CBRangeFromSV(POPs);
                    memcpy(returnBuffer, &s, returnSize);
                } else if (0 == strncmp(returnType, "{NSRect", strlen("{NSRect"))) {
...
#ifdef __i386__
double REAL_CBPerlIMP_fpret(id self, SEL _cmd, ...) {
    // Define a Perl context
    PERL_SET_CONTEXT(_CBPerlInterpreter);
    dTHX;
    ...
    if (success) {
        switch (*returnType) {
            ...
            default:
                NSLog(@"CBPerlIMP_fpret() called with return type %s", returnType);
                break;
        }
    } else {
        returnValue.dfloat = 0.0;
    }
